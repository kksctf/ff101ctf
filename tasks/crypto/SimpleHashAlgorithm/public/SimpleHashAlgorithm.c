...
void SimpleHashAlgorithmUpdate(SimpleHashAlgorithm_CTX *context, const unsigned char *data, u_int32_t len)
{
    ...
    if ((j + len) > 63)
    {
        ...
        SimpleHashAlgorithmTransform(context->state, context->buffer);
        for (; i + 63 < len; i += 64)
        {
            SimpleHashAlgorithmTransform(context->state, &data[i]);
        }
        j = 0;
    }
    else
...
void SimpleHashAlgorithmFinal(unsigned char digest[20], SimpleHashAlgorithm_CTX *context)
{
    ...
    c = 0200;
    SimpleHashAlgorithmUpdate(context, &c, 1);
    while ((context->count[0] & 504) != 448)
    {
        c = 0000;
        SimpleHashAlgorithmUpdate(context, &c, 1);
    }
    SimpleHashAlgorithmUpdate(context, finalcount, 8);
    for (i = 0; i < 20; i++)
    {
